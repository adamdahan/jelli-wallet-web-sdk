...
/**
 * Extract user avatar/profile picture from the OAuth session
 * @param {Object} session - The session object from loadSession()
 * @returns {string|null} - The profile picture URL or null if not found
 */
export function extractAvatar(session) {
  if (!session?.oauth) return null;

  const possibleAvatars = [
    session.oauth.profile?.avatarUrl,    // Primary location based on your structure
    session.oauth.profile?.picture,
    session.oauth.data?.profile?.picture,
    session.oauth.user?.picture,
    session.oauth.user?.photoURL,
    session.oauth.profile?.avatar_url,
    session.oauth.picture,
  ].filter(Boolean);
...
/**
 * Extract user's display name from the OAuth session
 * @param {Object} session - The session object from loadSession()
 * @returns {string|null} - The display name or null if not found
 */
export function extractDisplayName(session) {
  if (!session?.oauth) return null;

  const possibleNames = [
    session.oauth.profile?.displayName,  // Primary location based on your structure
    session.oauth.profile?.name,
    session.oauth.data?.profile?.name,
    session.oauth.user?.displayName,
    session.oauth.user?.name,
    session.oauth.name,
  ].filter(Boolean);
